...
def text_chunker(results: dict):
    ...
    chunks = chunker.create_documents([result["abstract"] for result in results if result.get("abstract")])
    return chunks


results = fetch_pmc_articles_by_query(query="diabetes", page_size=2)
docs=text_chunker(results)
embed_docs(docs)
